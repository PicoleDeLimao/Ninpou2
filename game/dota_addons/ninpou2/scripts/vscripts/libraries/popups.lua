...
function PopupHealing(target, amount)
    PopupNumbers(target, "heal", Vector(0, 255, 0),2.0, amount, POPUP_SYMBOL_PRE_PLUS, nil)
end
...
function PopupDamage(target, amount)
    PopupNumbers(target, "damage", Vector(255, 0, 0), 2.0, amount, nil, POPUP_SYMBOL_POST_DROP)
end

-- e.g. when dealing critical damage
function PopupCriticalDamage(target, amount)
    PopupNumbers(target, "crit", Vector(255, 0, 0), 2.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end

-- e.g. when taking damage over time from a poison type spell
function PopupDamageOverTime(target, amount)
    PopupNumbers(target, "poison", Vector(215, 50, 248), 2.0, amount, nil, POPUP_SYMBOL_POST_EYE)
end

-- e.g. when blocking damage with a stout shield
function PopupDamageBlock(target, amount)
    PopupNumbers(target, "block", Vector(255, 255, 255), 2.0, amount, POPUP_SYMBOL_PRE_MINUS, nil)
end
...
function PopupMiss(target)
    PopupNumbers(target, "miss", Vector(255, 0, 0), 2.0, nil, POPUP_SYMBOL_PRE_MISS, nil)
end
...
function PopupMana(target, amount)
    PopupNumbers(target, "heal", Vector(0, 176, 246), 2.0, amount, POPUP_SYMBOL_PRE_PLUS, nil)
end

function PopupHealthTome(target, amount)
    PopupNumbers(target, "miss", Vector(255, 255, 255), 2.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end

function PopupStrTome(target, amount)
    PopupNumbers(target, "miss", Vector(255, 0, 0), 2.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end

function PopupAgiTome(target, amount)
    PopupNumbers(target, "miss", Vector(0, 255, 0), 2.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end

function PopupIntTome(target, amount)
    PopupNumbers(target, "miss", Vector(0, 176, 246), 2.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end
